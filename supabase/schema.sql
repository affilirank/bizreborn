...
-- ------------------------------------------------------------
-- Lead generation module (audits + pitch videos)
--
-- ADDITIVE — safe to run on an existing biz-reborn database
-- (unlike the full schema above, which targets a fresh DB).
-- Creates the `prospects` pipeline table and the `leadgen`
-- storage bucket used by the admin /admin/prospects back-office.
-- ------------------------------------------------------------
...
-- Admin back-office manages prospects; public pitch pages read 'ready' rows.
...
-- Public pitch page: anyone can read a prospect that is 'ready'.
create policy "prospects read public when ready"
  on public.prospects for select
  to anon, authenticated
  using (status = 'ready');

-- Lead-gen media bucket (thumbnails, voiceovers, pitch videos).
